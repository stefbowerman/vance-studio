...
;(function($, Vance, Modernizr, undefined){
  ...
  function ProductImageZoomController(options){
    ...
    var $productPhotoMainLink = $productPhotoMain.find('a');
    var $productPhotoMainImg = $productPhotoMain.find('img');
    var $productBlowUp = $('.product-blowup');
    var $productBlowUpImage = $('.product-blowup__image');
    var $productBlowUpClose = $('.product-blowup__close');

    this.init = function(){
      settings = $.extend({}, defaults, options || {});

      settings.touch ? this.enableTouchZoom() : this.enableHoverZoom(); // jshint ignore:line
    ...
    this.hoverZoomOut = function(){
      if(zoomedIn){
        $productPhotoMain.trigger('click');  
      }
    };
    ...
    this.touchZoomIn = function(){
      ...
      $productBlowUpImage.attr('src', $productPhotoMainLink.attr('href'));
      $productBlowUp.css({
      ...
      iScroll = new IScroll($productBlowUp.get(0), {
        ...
        startX: $window.width()/2 - (initialZoom * 900)/2, // 900 is the zoomed image dimension
        startY: $window.height()/2 - (initialZoom * 900)/2 // 900 is the zoomed image dimension
      });
      if(canZoom){
        iScroll.enable();
      }
      $body.addClass('product-blowup-open');
      $productBlowUp.addClass('is-active');
      zoomedIn = true;
    };

    this.touchZoomOut = function(){
      console.log('closeImageZoom');
      
      if(!zoomedIn) return;

      if(iScroll instanceof IScroll){
        iScroll.destroy();
      }

      $productBlowUp.removeClass('is-active');
      $body.removeClass('product-blowup-open');
      zoomedIn = false;
    };

    this.enableTouchZoom = function(){
      ...
      $productPhotoMain.on('click', function(){
        console.log('calling touchZoomout from click.touch on productPhotoMain');
        _this.touchZoomIn.call(_this);
        return false;
      });

      $productBlowUp.on('click', function(){
        ...
        if (tapLength < 200 && tapLength > 0) {
          // double tap. don't care about it, let iscroll deal.
          // event.preventDefault();
        } else {
          // single tap.
          timeout = setTimeout(function() {
            console.log('calling touchZoomout from inside the setTimeOut');
            _this.touchZoomOut.call(_this);
            clearTimeout(timeout);
          }, 200);
        }
        lastTap = currentTime;
      ...
      $productBlowUpClose.on('click touchend', function(){
        console.log('calling touchZoomout from inside productBlowUpClose on click');
        _this.touchZoomOut.call(_this);
        return false; // Prevent it from bubbling or 'click' will be triggered on $productBlowUp
      });

    };

    this.destroyTouchZoom = function(){
      console.log('calling touchZoomout from inside destroyTouchZoom');
      this.touchZoomOut();
      $productPhotoMain.off('click');
      $productBlowUp.off('click');
      $productBlowUpClose.off('click touchend');
    };

    this.changeImage = function(src, srcZoom){
      if(settings.touch){
        this.destroyTouchZoom();
        $productPhotoMainImg.attr('src', src);
        $productPhotoMainLink.attr('href', srcZoom);
  ...
  /**
   * Used to control interaction of the thumbnail images on the product page
   *
   * @constructor
   * @param {Object} options - Configuration
   * @param {Function} options.onThumbClick - Called when a thumbnail is clicked, args - src, srcZoom
   * @return {self}
   */
  function ProductThumbsController(options){
    ...
    var $productThumbs = $('.product-photo-thumb');

    this.init = function(){
      settings = $.extend({}, defaults, options || {});
      $productThumbs.on('click', function(e){
        e.preventDefault();
        _this.onThumbClick($(this));
      });
      return this;
    };
    ...
    this.onThumbClick = function($thumb){
      ...
      if($thumb.hasClass( currentClass )){
        return;
      }

      var $link   = $thumb.find('a');
      var src     = $link.attr('href');
      var srcZoom = $link.attr('data-zoom');

      $productThumbs.removeClass( currentClass ); // remove from all
      $thumb.addClass( currentClass ); // add to clicked on one

      settings.onThumbClick(src, srcZoom);
    };
  ...
  $(function(){
    ...
    var productThumbsController = new ProductThumbsController({
